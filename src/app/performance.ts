...
export class PerformanceMonitor {
  ...
  private latestSimulationStepMs = 0
  ...
  constructor(sampleCapacity = 300) {
    if (!Number.isInteger(sampleCapacity) || sampleCapacity < 1) {
      throw new RangeError(
        'Performance sample capacity must be a positive integer',
      )
    }
  ...
  recordSimulationStep(simulationStepMs: number): void {
    if (!isNonNegativeFinite(simulationStepMs)) {
      throw new RangeError('Simulation timing must be finite and non-negative')
    }
    this.latestSimulationStepMs = simulationStepMs
  }
  ...
  recordRenderFrame(
    ...
    if (
      !isNonNegativeFinite(frameMs) ||
      !isNonNegativeFinite(instanceSyncMs) ||
      !Number.isInteger(activeParticles) ||
      activeParticles < 0 ||
    ...
    this.simulationSamples[this.nextSample] = this.latestSimulationStepMs
  ...
  snapshot(heapUsedBytes: number | null = null): PerformanceSnapshot {
    ...
    if (count === 0) {
      return {
        ...
        averageFrameMs: 0,
        p95FrameMs: 0,
        averageSimulationStepMs: 0,
        averageInstanceSyncMs: 0,
    ...
    const frames = Array.from(this.frameSamples.subarray(0, count)).sort(
      (a, b) => a - b,
    )
...
export function readBrowserHeapBytes(
  source: unknown = performance,
): number | null {
  if (typeof source !== 'object' || source === null || !('memory' in source))
    return null
...
export function createPerformanceReport(
  ...
  return {
    ...
    userAgent,
    buildMode,
    metrics: monitor.snapshot(heapUsedBytes),
  }
}
