# Documentation

This index separates current authority from historical delivery evidence.

## Start here

- [Architecture](ARCHITECTURE.md)
- [Implementation plan index](../IMPLEMENTATION_PLAN.md)
- [Detailed delivery plans](plans/README.md)
- [Commands and data contracts](CONTRACTS.md)
- [Dose-sweep regression contract](REGRESSION_CONTRACT.md)
- [Public-data and branding boundary](DATA_BOUNDARY.md)
- [Reference-library source record](REFERENCE_LIBRARY_SOURCES.md)

## Product, design, and model

- [Architecture decisions](DECISIONS.md)
- [Hybrid jar-test design direction](DESIGN_DIRECTION_JAR_TEST_HYBRID.md)
- [Original design brief](Sunol%20FlowLab%20VR%20Design%20Brief.pdf)
- [Visual behavior target](VISUAL_BEHAVIOR.md)
- [Display and interaction tuning record](TUNING.md)
- [Version 1 modeling research amendment](MODELING_RESEARCH_AMENDMENT.md)
- [Treatment-result ghost replay design](GHOST_REPLAY_DESIGN.md)
- [Post-v0.1 mechanistic research contract](POST_V1_MECHANISTIC_COAGULATION_RESEARCH.md)
- [Batch 12 California-first reference library](plans/batch-12-reference-library.md)

## Current validation and operations

- [UX and headset validation](UX_VALIDATION.md)
- [Performance evidence](PERFORMANCE.md)
- [Quest and browser testing](DEVICE_TESTING.md)
- [XR interaction evidence](XR_INTERACTION.md)
- [Asset provenance](ASSET_PROVENANCE.md)
- [Batch 10 environment budget](BATCH_10_ENVIRONMENT_BUDGET.md)

## Batch acceptance evidence

- [Batch 02A simulation acceptance](BATCH_02A_ACCEPTANCE.md)
- [Batch 03 progress record](BATCH_03_PROGRESS.md)
- [Batch 03 Workstream 03D technical acceptance](BATCH_03_03D_TECHNICAL_ACCEPTANCE.md)
- [Batch 03 final acceptance](BATCH_03_ACCEPTANCE.md)
- [Batch 03 recognition review packet](BATCH_03_REVIEW_PACKET.md)
- [Batch 04 interaction acceptance](BATCH_04_ACCEPTANCE.md)
- [Batch 05 integration acceptance](BATCH_05_ACCEPTANCE.md)
- [Batch 06 treatment-cycle acceptance](BATCH_06_ACCEPTANCE.md)
- [Batch 07 instrumentation and persistence acceptance](BATCH_07_ACCEPTANCE.md)
- [Batch 08 headset-readability candidate](BATCH_08_CANDIDATE.md)

## Evidence and archive

- [Batch 03 captured evidence](evidence/batch-03-review/)
- [Hosted v0.1 screenshot](images/sunol-flowlab-v0.1/shot-0.png)
- [v0.1.0 release notes](RELEASE_NOTES_V0.1.0.md)
- [Archived superseded material](archive/README.md)

Historical batch documents may describe the state when their gate was run.
[IMPLEMENTATION_PLAN.md](../IMPLEMENTATION_PLAN.md) and
[HANDOFF.md](../HANDOFF.md) contain the current release status.
